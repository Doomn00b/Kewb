...
func _physics_process(delta: float) -> void: #I picked physics, since this is movement.
	...
	if Input.is_action_just_pressed("jump") and is_on_floor():
		...
		is_grounded = false #We make sure to say the player isn't grounded.
	...
	_apply_movement() #We run the movement-state switch.
	move_and_slide()	
...
func _apply_gravity(delta : float):
	if not is_on_floor() : #Do I need MORE in the condition?
		...
		#print_debug("Applying gravity.")
	
	else: #Otherwise, we...
		return #...don't do anything. (no gravity-application)
	
func _apply_movement(): #Let's do a switch, aka a match, instead of 10k if-statements.
	match MovementState :
